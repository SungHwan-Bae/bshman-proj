...
def main():
    tests = """
       SELECT E.ENTNAME                              AS "엔터티명"
     , AA.ATTRNAME                            AS "부모속성명"
     , A.ATTRNAME                             AS "자식속성명"
        FROM DA_RELATIONSHIP R
        , DA_DERIVEDCONT D
        , DA_ATTRIBUTE A
        , DA_ATTRIBUTE AA
        , DA_ENTITY E
        WHERE 1=1
        AND R.REFAREAID       = D.REFAREAID
        AND R.RELID           = D.RELID
        AND R.EXPIREDDATE     = D.EXPIREDDATE
        AND D.REFAREAID       = A.REFAREAID
        AND D.REFDERIVEDID    = A.ATTRID
        AND D.EXPIREDDATE     = A.EXPIREDDATE
        AND R.EXPIREDDATE     = '99999999999999'
        AND D.REFAREAID       = AA.REFAREAID
        AND D.REFDERIVINGID   = AA.ATTRID
        AND D.EXPIREDDATE     = AA.EXPIREDDATE
        AND A.REFAREAID  = E.REFAREAID
        AND A.REFENTID   = E.ENTID
        AND AA.ATTRNAME = '프로젝트코드'
    """
    print("_____________")
    result = select_stmt.parseString(tests)
    ...
    #    print("struct : ", struct)
       ...
       if 'col' in struct:
        #    print("col22 : ", struct)
            for c in struct.col:
                print(c)
        #    columns_names.extend(c[0] for c in struct.col)
